...
int checkInOpen(STATE* newState, NODE **open){
	...
	while(temp!=NULL){
		...
		if(flag == 0){
			//printf("\nduplicate in open %d %d\n", newState->f, temp->list->f);
			check = 1;
			if(newState->f < temp->list->f ){
				//printf("opopop");
				/*temp->list->g = newState->g;
				temp->list->f = newState->f;
				temp->list->h = newState->h;
				temp->list->pos = newState->pos;*/
				temp->list->remove = 1;
		//		printf("UPDATED OPEN\n");
				return 1;
			}
...
int manhattanDistance(STATE* state){
	...
	for(int i=0;i<16;i++){
		int x1 = (i)/4;
		int y1 = (i)%4;
		int x2 = (state->current.p[i]-1)/4;
		int y2 = (state->current.p[i]-1)%4;

		//printf("%d %d %d %d  %d %d\n", x1,x2,y1,y2,i,state->current.p[i]);
		if(state->current.p[i]==0){
			count+=(3-x1)+(3-y1);
			//printf("count = %d\n", count );
		}
		else{
...
NODE *expand(STATE *best, NODE *open, NODE *closed){
	...
	if(best->pos/4 > 0){
		...
		newState->h = manhattanDistance(newState);
